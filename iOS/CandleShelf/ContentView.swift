...
struct ItemRow: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            Text("Scent: \(item.primaryField)")
                .font(Theme.captionFont)
                .foregroundColor(Theme.textMuted)
            if !item.secondaryField.isEmpty {
                Text("Burn Hours Remaining: \(item.secondaryField)")
                    .font(Theme.captionFont)
                    .foregroundColor(Theme.textMuted)
            }
            if !item.subLogs.isEmpty {
                Text("\(item.subLogs.count) Burn Rotation entries")
                    .font(Theme.captionFont)
        ...
        NavigationStack {
            Form {
                Section("Details") {
                    ...
                    TextField("Scent", text: $primaryField)
                        .accessibilityIdentifier("primaryFieldInput")
                    TextField("Burn Hours Remaining", text: $secondaryField)
                        .accessibilityIdentifier("secondaryFieldInput")
                    TextField("Notes", text: $notes, axis: .vertical)
                        .accessibilityIdentifier("notesField")
                }

                Section("Burn Rotation") {
                    if purchases.isPurchased {
                        ...
                        TextField("New burn session note", text: $newSubLogNote)
                        ...
                        Button("Add Burn Rotation Entry") {
                        ...
                        Button("Unlock Burn Rotation with Pro") {
                            showingPaywall = true
                        }
